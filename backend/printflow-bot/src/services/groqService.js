...
// Daftar stock nyata disisipkan ke system prompt tiap request, supaya AI
// mencocokkan sebutan customer (termasuk typo/istilah beda) ke nama barang
// yang benar-benar ada -- bukan menebak dari pengetahuan umum. Pencocokan
// nama ini satu-satunya bagian yang diserahkan ke AI; angka stock/harga
// tetap dibaca dari data asli, bukan dari jawaban AI (lihat pricingService).
//
// Info yang kurang (jumlah lembar belum disebut, produk belum jelas) SENGAJA
// tidak diserahkan ke AI untuk diputuskan -- pernah dicoba dan AI-nya malah
// balik nanya harga ke customer, yang jelas salah karena harga itu data toko
// sendiri. Sekarang pengecekan "info apa yang masih kurang" dilakukan
// deterministik di messageHandler.js, bukan ditebak AI.
function buatSystemPrompt(daftarStock) {
...
"oke fix", "lanjutkan aja pesanannya"). Isi false kalau customer masih
tanya-tanya harga/ketersediaan atau belum ada sinyal jelas mau memesan.
...
// Groq (mode JSON) menjamin hasilnya JSON valid secara sintaks, tapi tidak
// memaksa skema field seperti responseSchema Gemini. Field yang hilang atau
// salah tipe di sini dinormalisasi ke default yang aman.
...
async function analyzeMessage(customerMessage, daftarStock = [], riwayat = []) {
  const completion = await groq.chat.completions.create({
    model: env.groqModel,
    messages: [
      { role: 'system', content: buatSystemPrompt(daftarStock) },
      ...riwayat,
...
async function susunBalasanAlami(pesanCustomer, konteks, riwayat = []) {
  const systemPrompt = `Kamu adalah pegawai toko percetakan "${env.storeName}" yang membalas chat WhatsApp
...
Panduan khusus untuk situasi seputar pembayaran (karena ini menyangkut uang,
ikuti persis):
- "tanya_metode_pembayaran": tanyakan customer mau bayar pakai metode apa dari
  daftar di field opsiPembayaran (cash/qris/transfer). Kalau opsinya cuma
  ["cash"], jangan menawarkan pilihan -- langsung bilang pembayaran cuma bisa
  cash, datang ke toko kami.
- "kirim_info_qris": kasih tau customer gambar QRIS akan dikirim (gambarnya
  dikirim otomatis oleh sistem, bukan kamu yang menyebut isi gambarnya), lalu
  WAJIB minta customer bilang "sudah" dan kirim foto bukti pembayaran begitu
  selesai transfer, supaya bisa dicek admin.
- "kirim_info_rekening": sebutkan PERSIS nomor rekening & nama bank dari field
  daftarRekening di konteks (jangan pernah mengarang nomor rekening), lalu
  WAJIB minta customer bilang "sudah" dan kirim foto bukti pembayaran begitu
  selesai transfer, supaya bisa dicek admin.
...
- "menunggu_pilihan_metode_pembayaran": customer belum jelas jawab metode
  pembayaran yang mana -- tanya ulang dengan santai, jangan lanjut proses apa-apa.
- "order_dikonfirmasi_cash" / "order_dikonfirmasi_cash_only": order sudah
  tercatat, metode bayarnya cash di toko -- konfirmasi itu ke customer.
